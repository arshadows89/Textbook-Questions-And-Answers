...
actionList = ['feed', 'walk', 'putToBed', 'toss', 'rock']
puts 'What would you like to name your dragon?'
    @name = gets.chomp
   pet = Dragon.new @name

while @selfDead != false
x = gets.chomp.to_s
  if actionList.include? x
  pet.send x
  else
  puts'You have chosen a action that doesnt exist. Chose a action from the list below' 
  puts actionList
  end
end
